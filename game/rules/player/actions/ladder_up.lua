...
return function()
  return Thunk(actionTypes.LADDER_UP, function(dispatch, getState)
    local state = getState()
    local player = getPlayer(state)
    local entities = getByPosition(state, player.position)
    local ladder = tables.findFirst(entities, function(e) return e.ladderUp ~= nil end)

    if ladder then
      dispatch(move(player, Position.up(player.position)))
    end
  end)
end
